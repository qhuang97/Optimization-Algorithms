"""
Sample code automatically generated on 2022-12-06 14:47:44

by www.matrixcalculus.org

from input

d/dx 2*exp(-(x-x0)'*D*(x-x0)) * D * (x-x0) = 2*exp(-(x-x0)'*D*(x-x0))*D-4*exp(-(x-x0)'*D*(x-x0))*D*(x-x0)*((x-x0)'*D)
...
def fAndG(D, x, x0):
    ...
    t_3 = (2 * t_2)
    functionValue = (t_3 * t_1)
    gradient = ((t_3 * D) - ((4 * t_2) * np.outer(t_1, (t_0).dot(D))))
