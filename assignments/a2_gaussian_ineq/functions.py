"""
Sample code automatically generated on 2022-12-06 14:19:44

by www.matrixcalculus.org

from input

d/dx 2*exp(-(x-x0)' * D * (x-x0)) * (x-x0) = 2*exp(-(x-x0)'*D*(x-x0))*eye-4*exp(-(x-x0)'*D*(x-x0))*(x-x0)*((x-x0)'*D)
...
def fAndG(D, x, x0):
    ...
    t_0 = (x - x0)
    t_1 = np.exp(-(t_0).dot((D).dot(t_0)))
    t_2 = (2 * t_1)
    functionValue = (t_2 * t_0)
    gradient = ((t_2 * np.eye(x_rows, x_rows)) - ((4 * t_1) * np.outer(t_0, (t_0).dot(D))))
