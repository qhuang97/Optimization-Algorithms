    ...
    while True:
        ...
        for i in range(len(id_ineq)):
            if (g_ini[i] >= 0 or lamda[i] > 0):
                ...
                (J_g.T)[:,i] = 0
                # D_g[i] += 2 * (J[id_ineq].T)[i] * g_ini[i]  # J
        ...
        # k_h = np.dot(kappa, h_ini.T)
        # h_2 = nu * np.dot(h_ini, h_ini.T)
        # k_h = 0
        # h_2 = 0
        # if (len(h_ini) == 0):
        #     k_h = 0
        ...
        # S_ini_ges = f_ini + np.dot(r_ini, r_ini.T) + np.dot(lamda, g_ini.T) + mu * np.dot(g_ini, g_ini.T) + k_h + h_2
        ...
        while not found:
            ...
            #     h_k2 = 0
            # else:
            #     h_x = np.dot(kappa, h_k.T)
            #     h_k2 = nu * np.dot(h_k, h_k.T)
            # h_x = np.dot(kappa, h_k.T)
            # h_k2 = nu * np.dot(h_k, h_k.T)
            # S_k_ges =  f_k + np.dot(r_k, r_k.T) + np.dot(lamda, g_k.T) + g_k_square + h_k2 + h_x
